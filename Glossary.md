...
## Hash-Based Signature Schemes (16강 Lamport 스킴의 한계와 해법)

- **해시 체인 (Hash Chain)**: 사전영상 저항적 함수 $G$를 무작위 초깃값 $x_0$에 반복 적용해 만든 수열 $x_0, x_1=G(x_0), \dots, x_M=G^M(x_0)$. 어떤 원소 $x_i$가 주어지면 뒤쪽 원소($j>i$)는 $G$를 더 적용해 쉽게 얻지만, 앞쪽 원소($j<i$)는 $G$의 역상을 구해야 하므로 계산 불가능하다. 이 "앞으로만 이동 가능"이라는 단방향성이 Winternitz 서명의 안전성 토대다.
- **Winternitz 일회성 서명 스킴 (Winternitz OTS) / Winternitz 파라미터 (Winternitz parameter, $w$)**: 메시지 해시 $H(M)$의 비트를 $w$비트 블록 단위로 서명하는 일회성 서명 스킴(Winternitz, 1979). 블록마다 길이 $2^w-1$의 해시 체인 하나를 두고, 블록 값 $h_i$만큼 체인을 전진한 원소를 서명 성분으로 공개한다. Lamport 대비 공개키·서명이 크게 작아지지만($n=256, w=8$에서 16,384/8,192바이트 → 각 1,088바이트) 체인당 최대 $2^w-1$회의 $G$ 평가가 필요해 서명·검증이 느리다. $w$가 이 크기↔속도 트레이드오프를 조절한다.
- **체크섬 (Checksum, Winternitz)**: $\operatorname{csum}(h) = \sum_i (2^w - 1 - h_i)$ — 각 항은 $i$번째 체인에서 서명 성분과 끝값 사이의 남은 거리다. 이 값을 $w$비트 블록으로 쪼개 추가 해시 체인으로 함께 서명한다. 메시지 블록을 하나라도 키우면 체크섬은 반드시 줄어들고, 체크섬 블록을 줄이려면 체인을 뒤로 가야 하므로(불가능), "더 큰 해시를 갖는 메시지로의 위조"가 차단된다. 체크섬이 없으면 Winternitz OTS는 안전하지 않다.
- **Merkle 해시 트리 (Merkle Hash Tree) / 인증 경로 (Authentication Path)**: 다수의 일회성 공개키 $Y_i$를 하나의 짧은 장기 공개키로 인증하는 완전 이진 트리. 리프에 $F(Y_i)$를, 내부 노드에 두 자식 값의 연접 해시를 배정해 루트 $R$까지 쌓으며, $R$이 장기 공개키가 된다. 특정 $Y_i$의 진위는 리프에서 루트까지 경로상 형제 노드 값들(=인증 경로, 높이 $d$ 트리에서 $d$개)로 루트를 재구성해 확인한다. 안전성은 $F$의 충돌 저항성에 의존한다.
- **하이퍼트리 / 멀티트리 (Hypertree / Multi-tree)**: 높이 $D$의 큰 Merkle 트리를 여러 층의 작은 트리로 분할한 구조. 상위 층 리프의 OTS 개인키가 하위 층 트리의 루트에 서명하고, 최하위 층 리프의 OTS 개인키가 실제 메시지에 서명한다. 층 1 트리만 만들면 장기 공개키가 정해지고 하위 층은 필요할 때 생성하므로 키 생성이 크게 빨라진다. 대가는 서명에 OTS 서명이 층 수만큼 들어가 길어지는 것. 서로 다른 암호 모듈에 하위 트리를 분산할 수 있다. LMS·SPHINCS+가 사용한다.

## Hash-Based Signature Schemes (17강 Leighton-Micali 서명 스킴(LMS)과 LM-OTS)

- **LM-OTS (Leighton-Micali One-Time Signature scheme)**: Winternitz OTS에 안전성 보강을 더한 일회성 서명 스킴. 체인 끝값 목록 $Y$ 대신 그 해시 $K$ 하나를 공개키로 쓰고(검증자가 서명에서 $Y$를 재구성해 K와 비교), 체인·메시지 해싱 모두에 단일 해시 함수 $H$를 고유 접두사와 함께 쓰며, 메시지에 무작위화 문자열을 덧붙여 해싱한다. LMS·HSS가 이 일회성 서명을 Merkle 트리 리프에 배치해 다회 서명 스킴을 만든다.
- **고유 접두사 (Unique Prefix)**: LM-OTS가 모든 해시 호출 앞에 붙이는, 그 호출의 맥락(트리 식별자 $I$, 리프 번호 $q$, 체인 인덱스 $i$, 체인 위치 $j$, 목적별 상수)을 유일하게 식별하는 값. 서로 다른 목적의 해시 호출을 입력 수준에서 완전히 독립시켜, 하나의 해시 함수를 여러 용도로 재사용해도 형식적 안전성 증명(랜덤 오라클 모델)이 가능하게 한다.
- **메시지 무작위화 문자열 (Message Randomizer)**: 서명자가 서명할 때마다 새로 뽑아 메시지 앞에 붙여 함께 해싱하는 무작위 문자열 $C$(LM-OTS는 $n$비트). 서명에 포함돼 검증에 쓰인다. 공격자가 서명 전에 $C$ 값을 알거나 통제할 수 없으므로, 메시지 해시에 쓰이는 해시 함수가 충돌 저항적일 필요 없이 사전영상 저항적이기만 하면 안전성이 성립한다.
- **랜덤 오라클 모델 / 양자 랜덤 오라클 모델 (Random Oracle Model, ROM / Quantum Random Oracle Model, QROM)**: 안전성 증명에서 해시 함수를 입력마다 완전히 무작위인 값을 돌려주는 이상화된 공개 함수로 취급하는 분석 틀(ROM). QROM은 여기서 한 걸음 더 나아가 공격자가 오라클을 중첩 상태로(양자적으로) 질의할 수 있다고까지 허용하는, 더 강한 분석 틀이다. LMS는 2016년 Katz가 ROM에서, 2018년 Eaton이 QROM에서 안전성을 증명했다.
- **비트마스크 (Bitmask, XMSS)**: XMSS가 해시 함수 입력에 XOR로 섞어 넣는 의사난수 비트열. LMS의 고유 접두사(값을 덧붙이는 방식)와 달리 입력을 뒤섞는 방식이며, 이 덕분에 XMSS의 안전성 증명은 해시 함수를 랜덤 오라클로 이상화하지 않는 표준 모델(standard model)에서 성립한다.
